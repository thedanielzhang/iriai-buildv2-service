...
<p class="sd-overview-text">Create a new standalone HTML page (charmander.html) at the project root that displays Charmander ASCII art with a fire-themed visual style (dark background, orange/red text). The page follows the same technical approach as the existing Pikachu page — static HTML, inline CSS, centered monospace pre block — but with its own distinct aesthetic. The existing index.html must not be modified.</p>
</section>
